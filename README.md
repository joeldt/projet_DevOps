...
🎯 Objectifs du Projet
Développement Applicatif : Création d'un portfolio simple et élégant présentant mes compétences.
...
Formulaire : Body-parser (gestion des données), Nodemailer (envoi d'emails)
...
À chaque git push sur la branche main, le pipeline exécute les étapes suivantes :

Installation : npm install pour récupérer les dépendances (Express, Jest, etc.).

Tests Unitaires : Vérification des routes (Accueil, Envoi de formulaire, Erreur 404).

Docker Build : Création d'une image Docker pour isoler l'application.

Tagging : Création automatique d'une version (ex: v1.0.12) en cas de succès.
...
Développé par Joël - Projet DevOps 2026
